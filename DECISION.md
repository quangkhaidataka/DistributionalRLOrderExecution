...
- **Per the FALLBACK RULE, recommend ONE rerun at the pre-declared fallback (0.05, 0.12) — NOT launched, reported, stopped.** No re-scan / no new-cell search (that would be post-hoc tuning). *Assessment:* the fallback may resolve DDQN's stochastic collapse but likely not the weak differentiation, which is driven by best-by-val-CVaR selecting an already-tail-aggressive IQN-neutral (little room for τ-truncation to improve) — a structural caveat, not a σ-value issue.

## JD diagnostics D1/D2 (2026-07-17) — selection-rule × σ, no decisions

- Ran two cheap diagnostics (added `--select-by {cvar,mean}` to `run_jd_staged --assemble-eval`; IQN-neutral only, DQN/DDQN stay cvar; `_mean` outputs kept side-by-side). **D1:** mean-select the existing σ=0.16 checkpoints. **D2:** full fallback retrain at σ=0.12 (fresh dir `results/_seeds/seed42/jump_diffusion_sig012`, σ=0.16 untouched), assemble+sweep both ways. Full 2×2 at `results/_jd_diagnostics/decision_matrix.md`.
- **All four cells FAIL the gate.** (1) **DDQN dump-collapses in every cell** (Std 0.009 at σ=0.16, 0.000 at σ=0.12) — σ- and selection-independent → a DDQN training instability, not a calibration lever; triggers the fallback rule everywhere. (2) **No robust headline-α differentiation**: the *controlled* Δ (IQN-neutral − IQN-CVaR₀.₉₅ CVaR₉₅, same eval RNG) is positive only for **σ=0.16-mean (+0.49)**, but its separate-seed α-sweep shows the opposite sign → within noise; and that cell's neutral CVaR₉₅ (14.1) exceeds TWAP (12.6). The other three cells have negative Δ (τ-truncation slightly worsens the tail). Only σ=0.16-cvar's α-sweep runs in the theoretically-correct (mild ↓) direction.
- **What selection controls:** how fast the *shared* neutral trades (mean-first-fraction), not whether CVaR helps. cvar → fast, tail-aggressive neutral (excellent absolute CVaR, no room to differentiate); mean → slow neutral (differentiates in one protocol, but tail worse than TWAP). **IQN-neutral never dumps** in any cell (symmetric calibration works for IQN).
- **Recommended cell (flagged, NO change taken): σ=0.16-cvar** — the current locked default, only cell with strong absolute IQN tail (−73% vs TWAP) AND a correct-direction sweep, but it does not show headline-α differentiation. **Open question for the user:** the "zero-cost CVaR control" mechanism is not robustly visible at α=0.90/0.95 in this JD setup under any of {σ, selection}; the differences are eval-seed noise. Levers to discuss (not auto-apply): report the α=0.5 frontier point where the effect is largest; different value-distribution stress; or accept σ=0.16-cvar's absolute result and frame the JD as a robustness/absolute-performance case rather than a differentiation showcase. **Decision deferred to the user.**
